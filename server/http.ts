...
export async function handleApi(req: IncomingMessage, res: ServerResponse) {
  try {
    ...
    if (method === 'POST' && path === '/locations') return send(res, 200, await saveLocation(await readBody(req), companyId, undefined, user.id));
    if (method === 'PUT' && parts[0] === 'locations' && parts[1]) return send(res, 200, await saveLocation(await readBody(req), companyId, parts[1], user.id));
    if (method === 'DELETE' && parts[0] === 'locations' && parts[1]) return send(res, 200, await deleteLocation(parts[1], companyId));
    if (method === 'POST' && path === '/warehouses') return send(res, 200, await saveWarehouse(await readBody(req), companyId, undefined, user.id));
    if (method === 'PUT' && parts[0] === 'warehouses' && parts[1]) return send(res, 200, await saveWarehouse(await readBody(req), companyId, parts[1], user.id));
    ...
    if (method === 'POST' && parts[0] === 'inbound' && parts[2] === 'cancel') return send(res, 200, await cancelInbound(parts[1], companyId, user.id));
